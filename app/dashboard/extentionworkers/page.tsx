...
function Page() {
  ...
  const handleCreate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const requiredFields = ['first_name', 'last_name', 'email', 'phone_number', 'age', 'address', 'birthday', 'religion', 'maritalStatus','photo'];
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="mr-14">
          <button className="flex items-center bg-secondary p-3 rounded-2xl gap-3" onClick={openModal}>
            <AddIcon /> <p className="text-white"> Add New Worker</p>
          </button>
        </div>
      </div>
      <MidNav />
      <Table headers={headers} data={extensionData} />
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        heading={"Add New Worker"}
      >
        <p className="mt-5 font-semibold">Worker Information</p>
        <hr />
